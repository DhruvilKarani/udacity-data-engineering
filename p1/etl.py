...
def process_song_file(cur, filepath):
    ...
    # insert song record
    song_data = [df.song_id.tolist()[0], df.title.tolist()[0], df.song_id.tolist()[0], df.year.tolist()[0], df.duration.tolist()[0]]
...
#     t = pd.DataFrame()
    ...
    for index, row in df.iterrows():
        ...
        songplay_data = (index, row.time_data, row.userId, row.level, songid, artistid, row.sessionId, row.location, row.userAgent)
